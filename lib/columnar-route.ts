...
export function columnarEncryptLines(
  stackedLines: string[],
  key: string
): string[] {
  ...
  for (const orderedColumnIx in keyChars) {
    ...
    for (const rowIx in stackedLines) {
      encryptedRows[rowIx][orderedColumnIx] =
        stackedLines[rowIx][messageColumnIx] ?? ' ';
    }
  }
...
export function columnarDecryptLines(
  stackedLines: string[],
  key: string
): string[] {
  const keyChars = key.split('');
...
export function routeEncryptArrange(
  message: string,
  ...
  const oddChunkSize = mainChunkSizes.find(
...
export function routeEncryptReadOut(
  stackedLines: string[],
  blockSize: number
): string {
...
export function routeDecryptReadIn(
  encrypted: string,
  rectangleSize: number,
  key: string
): string[] {
  ...
  for (let vLineIx = 0; vLineIx < blockSize; vLineIx++) {
    ...
    const currentVLineLength =
      transposedVLineIx < numberOfLongVLines
        ? longVLineLength
        : shortVLineLength;
    ...
    for (let rowIx = 0; rowIx < longVLineLength; rowIx++) {
      ...
      stackedLines[rowIx][vLineIx] =
        rowIx < currentVLineLength ? encrypted[encryptedIx++] : ' ';
    }
  }
...
export function routeDecryptUnarrange(
  stackedLines: string[],
  rectangleSize: number,
  blockSize: number
): string {
  ...
  for (let columnIx = 0; columnIx < blocksPerRow; columnIx++) {
    const rowsInBlock =
      columnIx < lastRowBlocks ? numberOfRows : numberOfRows - 1;
...
export function routedColumnarEncrypt(
  message: string,
  rectangleSize: number,
  key: string
): string {
...
export function routedColumnarDecrypt(
  message: string,
  rectangleSize: number,
  key: string
): string {
  const unreversed = message.split('').reverse().join('');
  ...
  return routeDecryptUnarrange(columnarDecrypted, rectangleSize, key.length);
}
